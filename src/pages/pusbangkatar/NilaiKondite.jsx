...
function NilaiKondite() {
    ...
    const [nilaiMap, setNilaiMap] = useState({}) // mahasiswaId -> nilai_kondite
    ...
    // Current value (edited or from DB)
    ...
    const handleExport = () => {
        const exportData = filteredMahasiswa.map((m, idx) => ({
            ...
            prodi: m.prodi?.kode || '-',
            kelas: m.kelas?.nama || '-',
            nilai_kondite: getCurrentNilai(m)
        }))
        const headers = [
            { key: 'no', label: 'No' },
            { key: 'nim', label: 'NIM' },
            ...
            { key: 'nilai_kondite', label: 'Nilai Kondite (0-4)' }
        ]
        exportToXLSX(exportData, headers, `nilai_kondite_${tahunAkademik.replace(/\//g, '-').replace(/ /g, '_')}`, 'Nilai Kondite')
    }

    // Template
    const handleDownloadTemplate = () => {
        const templateHeaders = ['NIM', 'Nama', 'Nilai Kondite (0-4)']
        const sampleRows = filteredMahasiswa.slice(0, 3).map(m => [m.nim_nip || '', m.nama || '', ''])
        const infoRows = [
            [`# Tahun Akademik: ${tahunAkademik}`, '', ''],
            ['# Isi kolom "Nilai Kondite" dengan angka 0-4', '', '']
        ]
        downloadTemplate(templateHeaders, sampleRows, `template_kondite_${tahunAkademik.replace(/\//g, '-')}`, infoRows)
    }
    ...
    const handleImport = (e) => {
        ...
        importFromFile(file, ({ headers, rows, error }) => {
            ...
            rows.forEach(row => {
                ...
                const nilai = row['nilai kondite (0-4)'] || row['nilai_kondite'] || row['nilai kondite'] || row['nilai'] || ''
                if (!nim || nilai === '') return
